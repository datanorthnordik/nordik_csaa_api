...
func TestNormalizeSavePageSectionRequestAcceptsVideoModule(t *testing.T) {
	videoPackageID := 21

	section, err := normalizeSavePageSectionRequest(
		SavePageSectionRequest{
			SectionType: PageSectionTypeVideo,
			Video: &PageVideoSectionInput{
	...
	if err != nil {
		t.Fatalf("expected video module payload to be accepted, got %v", err)
	}
	if section.Video == nil || section.Video.VideoPackageID == nil || *section.Video.VideoPackageID != videoPackageID {
		t.Fatalf("expected normalized video payload, got %#v", section.Video)
	}
}

func TestNormalizeSavePageSectionRequestRejectsInvalidVideoPackageID(t *testing.T) {
	videoPackageID := 0
	...
	if err == nil || err.Error() != "page_detail.sections[0].video.video_package_id must be a positive integer" {
		t.Fatalf("expected invalid video package id error, got %v", err)
	}
}
...
func TestGetPageContentDetailIncludesHeaderTextAlign(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT .* FROM "page_section_video_modules" WHERE page_section_id IN \(\$1\)`).
		WithArgs(42).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "video_package_id", "created_at", "updated_at",
		}))
...
func TestGetPageContentDetailIncludesVideoModule(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	mock.ExpectQuery(`SELECT .* FROM "page_sections" WHERE page_detail_id = \$1 ORDER BY sort_order ASC.*id ASC`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			42, 5, "Video Module", PageSectionTypeVideo, 0, true, "{}", createdAt, updatedAt,
		))
	mock.ExpectQuery(`SELECT .* FROM "page_section_header_modules" WHERE page_section_id IN \(\$1\)`).
		WithArgs(42).
		WillReturnRows(sqlmock.NewRows([]string{
	...
	if resp.Sections[0].Video == nil || resp.Sections[0].Video.VideoPackageID == nil || *resp.Sections[0].Video.VideoPackageID != 21 {
		t.Fatalf("expected video section response, got %#v", resp.Sections[0])
	}
...
func TestGetPageContentDetailHandlesLegacyNullModuleColumns(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	mock.ExpectQuery(`SELECT .* FROM "page_sections" WHERE page_detail_id = \$1 ORDER BY sort_order ASC.*id ASC`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			AddRow(42, 5, "Header Module", PageSectionTypeHeader, 0, true, "{}", createdAt, updatedAt).
			AddRow(43, 5, "Typography", PageSectionTypeTypography, 1, true, "{}", createdAt, updatedAt).
			AddRow(44, 5, "Gallery Module", PageSectionTypeGallery, 2, true, "{}", createdAt, updatedAt).
			AddRow(45, 5, "Quote Module", PageSectionTypeQuote, 3, true, "{}", createdAt, updatedAt).
			AddRow(46, 5, "CTA Banner", PageSectionTypeCTABanner, 4, true, "{}", createdAt, updatedAt),
		)
	mock.ExpectQuery(`(?s)SELECT .*COALESCE\(main_header_text, ''\) AS main_header_text.*COALESCE\(NULLIF\(TRIM\(hierarchy\), ''\), 'h1_hero'\) AS hierarchy.*COALESCE\(NULLIF\(TRIM\(text_align\), ''\), 'left'\) AS text_align.*COALESCE\(underline_enabled, FALSE\) AS underline_enabled.* FROM "page_section_header_modules" WHERE page_section_id IN \(\$1,\$2,\$3,\$4,\$5\)`).
		WithArgs(42, 43, 44, 45, 46).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "main_header_text", "sub_header_text", "description", "hierarchy", "text_align", "underline_enabled", "created_at", "updated_at",
		}).AddRow(
			42, "Legacy Header", "", "", PageHeaderHierarchyHero, PageTextAlignLeft, false, createdAt, updatedAt,
		))
	mock.ExpectQuery(`(?s)SELECT .*COALESCE\(body_html, ''\) AS body_html.*COALESCE\(body_text, ''\) AS body_text.*COALESCE\(NULLIF\(TRIM\(text_align\), ''\), 'left'\) AS text_align.* FROM "page_section_typography_modules" WHERE page_section_id IN \(\$1,\$2,\$3,\$4,\$5\)`).
		WithArgs(42, 43, 44, 45, 46).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "body_html", "body_text", "text_align", "created_at", "updated_at",
		}).AddRow(
			43, "<p>Legacy copy</p>", "", PageTextAlignLeft, createdAt, updatedAt,
		))
	mock.ExpectQuery(`(?s)SELECT .*COALESCE\(NULLIF\(TRIM\(view_mode\), ''\), 'grid'\) AS view_mode.*COALESCE\(show_title_description, TRUE\) AS show_title_description.*COALESCE\(auto_scroll_enabled, FALSE\) AS auto_scroll_enabled.* FROM "page_section_gallery_modules" WHERE page_section_id IN \(\$1,\$2,\$3,\$4,\$5\)`).
		WithArgs(42, 43, 44, 45, 46).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "gallery_id", "view_mode", "show_title_description", "auto_scroll_enabled", "created_at", "updated_at",
		}).AddRow(
			44, 14, PageGalleryViewGrid, true, false, createdAt, updatedAt,
		))
	mock.ExpectQuery(`SELECT .* FROM "page_section_video_modules" WHERE page_section_id IN \(\$1,\$2,\$3,\$4,\$5\)`).
		WithArgs(42, 43, 44, 45, 46).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "video_package_id", "created_at", "updated_at",
		}))
	mock.ExpectQuery(`(?s)SELECT .*COALESCE\(quote_content, ''\) AS quote_content.*COALESCE\(attribution, ''\) AS attribution.* FROM "page_section_quote_modules" WHERE page_section_id IN \(\$1,\$2,\$3,\$4,\$5\)`).
		WithArgs(42, 43, 44, 45, 46).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "quote_content", "attribution", "created_at", "updated_at",
		}).AddRow(
			45, "Legacy quote", "", createdAt, updatedAt,
		))
	mock.ExpectQuery(`(?s)SELECT .*COALESCE\(banner_heading, ''\) AS banner_heading.*COALESCE\(banner_message, ''\) AS banner_message.*COALESCE\(button_text, ''\) AS button_text.*COALESCE\(button_url, ''\) AS button_url.*COALESCE\(open_in_new_tab, FALSE\) AS open_in_new_tab.*COALESCE\(image_url, ''\) AS image_url.*COALESCE\(image_object_key, ''\) AS image_object_key.* FROM "page_section_cta_banner_modules" WHERE page_section_id IN \(\$1,\$2,\$3,\$4,\$5\)`).
		WithArgs(42, 43, 44, 45, 46).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "banner_heading", "banner_message", "button_text", "button_url", "open_in_new_tab", "image_url", "image_object_key", "created_at", "updated_at",
		}).AddRow(
			46, "Legacy CTA", "", "", "", false, "", "", createdAt, updatedAt,
		))
	mock.ExpectQuery(`SELECT .*page_section_documents.*JOIN documents ON documents.id = page_section_documents.document_id.*page_section_documents.page_section_id IN \(\$1,\$2,\$3,\$4,\$5\).*`).
		WithArgs(42, 43, 44, 45, 46).
		WillReturnRows(sqlmock.NewRows([]string{
			"page_section_id", "document_id", "display_name", "description", "original_file_name", "file_url", "gcp_object_key", "mime_type", "file_size", "sort_order", "created_at", "updated_at",
		}))
	...
	resp, err := service.getPageContentDetail(12)
	if err != nil {
		t.Fatalf("expected legacy null module columns to be tolerated, got %v", err)
	}
	if resp == nil || len(resp.Sections) != 5 {
		t.Fatalf("expected five sections in content detail response, got %#v", resp)
	}
	if resp.Sections[0].Header == nil || resp.Sections[0].Header.TextAlign != PageTextAlignLeft || resp.Sections[0].Header.Hierarchy != PageHeaderHierarchyHero {
		t.Fatalf("expected header defaults to be applied, got %#v", resp.Sections[0].Header)
	}
	if resp.Sections[2].Gallery == nil || resp.Sections[2].Gallery.ViewMode != PageGalleryViewGrid || !resp.Sections[2].Gallery.ShowTitleDescription || resp.Sections[2].Gallery.AutoScrollEnabled {
		t.Fatalf("expected gallery defaults to be applied, got %#v", resp.Sections[2].Gallery)
	}
	if resp.Sections[3].Quote == nil || resp.Sections[3].Quote.Attribution != "" {
		t.Fatalf("expected empty quote attribution default, got %#v", resp.Sections[3].Quote)
	}
	if resp.Sections[4].CTABanner == nil || resp.Sections[4].CTABanner.ButtonText != "" || resp.Sections[4].CTABanner.ButtonURL != "" || resp.Sections[4].CTABanner.OpenInNewTab {
		t.Fatalf("expected CTA nulls to coalesce safely, got %#v", resp.Sections[4].CTABanner)
	}
